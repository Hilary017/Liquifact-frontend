...
import { fetchWithRetry, sleep, defaultDelay, defaultShouldRetry, isIdempotentMethod } from './fetchWithRetry';
...
describe('fetchWithRetry', () => {
  ...
  describe('abort signal handling', () => {
    it('throws AbortError if the original signal is already aborted', async () => {
      ...
      await expect(
        fetchWithRetry('/health', { signal: controller.signal }),
    ...
    it('stops retrying if the signal is aborted during retry delay', async () => {
      ...
      const promise = fetchWithRetry('/health', { signal: controller.signal }, { maxAttempts: 3, baseDelayMs: 1000 });
    ...
    it('does not retry AbortError from in-flight fetch', async () => {
      ...
      await expect(
        fetchWithRetry('/health', {}, { maxAttempts: 3 }),
      ).rejects.toThrow(DOMException);
